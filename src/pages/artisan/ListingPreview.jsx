...
export function ListingPreview() {
  ...
  useEffect(() => {
    ...
    async function loadArtisan() {
      try {
        ...
        let data =
          await artisanService.getArtisanById(artisanId);

        if (!data && artisanId !== 'a1') {
          data =
            await artisanService.getArtisanById('a1');
        }
  ...
  const catalog = (() => {
    try {
      return (
        JSON.parse(
          sessionStorage.getItem('ss_catalog')
        ) || mockAIGeneratedCatalog
      );
    } catch {
  ...
  const price =
    sessionStorage.getItem('ss_price') ||
    mockAIGeneratedCatalog.suggestedPrice?.recommended ||
    2200;

  /*
   * Prefer the AI-enhanced Cloudinary image.
   * If enhancement is unavailable, fall back to
   * the original photo captured by the artisan.
   */
  const originalPhoto =
    sessionStorage.getItem('ss_photo_preview') || null;

  const enhancedPhoto =
    sessionStorage.getItem('ss_enhanced_photo_url') || null;

  const previewPhoto =
    enhancedPhoto || originalPhoto;
    ...
    try {
      ...
      const created =
        await productService.createProduct({
          ...
          /*
           * This now points to the Cloudinary
           * enhanced image whenever available.
           */
  ...
  return (
    <div
      style={{
        ...
        background: 'var(--color-bg)',
        paddingBottom:
          'calc(var(--bottom-nav-height, 64px) + env(safe-area-inset-bottom, 0px) + var(--space-8, 32px))',
      }}
    >
      {/* Top Bar */}
      <div
      ...
      <div
        className="listing-preview-container"
        style={{
          maxWidth: 900,
          margin: '0 auto',
          padding:
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {isHindi
                ? 'पूर्वावलोकन: खरीदारों को आपकी लिस्टिंग ऐसे दिखेगी'
                : 'Preview Mode: This is how buyers will see your listing'}
            </span>
          </div>
          ...
          >
            {t(
              'artisan.qualityExcellent'
            )}
          </span>
        </div>

        {/* AI Enhanced Image Notice */}
        {enhancedPhoto && (
          <div
            style={{
              background:
          ...
          >
            <Sparkles size={14} />
            {isHindi
              ? 'AI द्वारा फोटो बेहतर बनाई गई है'
        ...
        <div
          className="listing-preview-grid"
          style={{
            display: 'grid',
            background:
              'var(--color-surface)',
          ...
          >
              ...
              >
                ...
                >
                  ...
                  catalog.descriptionHindi
                    ...
                    : catalog.description ||
                      (isHindi
                        ? 'पारंपरिक कारीगरों द्वारा हस्तनिर्मित उत्कृष्ट शिल्प।'
                        : 'Authentic handcrafted artisanal creation made with traditional techniques.')}
                </p>
              </div>
            ...
            >
              ...
              >
                {publishing
                  ? t(
                      'artisan.publishingToMarketplace'
                    )
                  : t(
                      'artisan.publishToMarketplace'
                    )}
              </button>
